...
#include "ss_proto/game_online_msg.h"
...
#include "error_code/error_code.pb.h"
...
CORO_STATE CLoginCoro::coro_process()
{
    ...
    //check user login uniq
    {
		...
		nRetCode = send_server_msg_by_routerid_coro(m_qwUserID, 1, g2o_user_login, &msg, sizeof(msg));
        LOG_PROCESS_ERROR(nRetCode);
    }
...
CORO_STATE CCreateRoleCoro::coro_process()
{
...

    {
        ...
        strxcpy(RoleBase.szRoleName, m_pRole->get_name(), MAX_USER_ROLE_NAME_LEN);
...
CORO_STATE CKickUserCoro::coro_process()
{
    ...
    if (m_pUser)
    {
        ...
        // logout from online server
        {
            ...
            nRetCode = send_server_msg_by_routerid_coro(m_qwUserID, 1, g2o_user_logout, &msg, sizeof(msg));
            LOG_PROCESS_ERROR(nRetCode);
        }
